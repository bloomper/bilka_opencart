...
class Modelmodulegafenh extends Model {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->modifypermission() && substr(VERSION,0,3)<='3.0') {
		...
		foreach($data['stores'] as $store) {
			...
			if(! isset($data[$this->modname.'_setting'][$store['store_id']]['cmv2']) ) {
				$data[$this->modname.'_setting'][$store['store_id']]['cmv2'] = 0;
			}
			...
			$html[] = $lang['entry_cmv2_ga_ads'];
			
			$name = sprintf($this->modname.'_setting[%s][%s]', $store['store_id'], 'cmv2');
			$val = $data[$this->modname.'_setting'][$store['store_id']]['cmv2'];
			$html[] = $this->get_RDO_html($name, $val, $lang['entry_cmv2'], array(1=>$data['text_yes'], 0=>$data['text_no'])); 
			
			$html[] = $lang['entry_ga4'];
			
			$name = sprintf($this->modname.'_setting[%s][%s]', $store['store_id'], 'gmid');
			$val = $data[$this->modname.'_setting'][$store['store_id']]['gmid'];
			$html[] = $this->get_InpTxt_html($name, $val, $lang['entry_gmid'], '');
			...
			$html[] = $this->get_InpTxt_html($name, $val, $lang['entry_adwid'], '');
			...
			$html[] = $this->get_InpTxt_html($name, $val, $lang['entry_adwlbl'], '');
...
/***************************************** HTML VIEW *************************************************/		
		...
		if(substr(VERSION,0,3)=='4.0' || substr(VERSION,0,3)=='4.1') {
			$html[] = sprintf('<div class="float-end"> <button type="submit" form="form-gafenh" class="btn btn-primary"><i class="fa-solid fa-save"></i></button> <a href="%s" class="btn btn-light"><i class="fa-solid fa-reply"></i></a></div>', $data['cancel']);
		} else {
			$html[] = sprintf('<div class="pull-right"> <button type="submit" form="form-gafenh" class="btn btn-primary"><i class="fa fa-save"></i></button> <a href="%s" class="btn btn-default"><i class="fa fa-reply"></i></a></div>', $data['cancel']);			
		}
		...
		if(substr(VERSION,0,3)=='4.0' || substr(VERSION,0,3)=='4.1') {
			$html[] = sprintf('<form action="%s" method="post" id="form-gafenh" data-oc-toggle="ajax">', $data['action']);			
			...
			$html[] = sprintf('<form action="%s" method="post" enctype="multipart/form-data" id="form-gafenh" class="form-horizontal">', $data['action']);
	...
	public function addmenu($data) {
		$menunm = 'Google Analytics 4 + Adwords Enhanced Conversion Tracking';
		$menulink = (substr(VERSION,0,3)=='4.0' || substr(VERSION,0,3)=='4.1') ? 'extension/gafenh/extension/gafenh' : 'extension/gafenh';		
	...
	public function install() { 		
		$query = $this->db->query("SHOW COLUMNS FROM `".DB_PREFIX."order` LIKE 'gafenh_ordflag' ");
		if(!$query->num_rows){
			$this->db->query("ALTER TABLE `".DB_PREFIX."order` ADD `gafenh_ordflag` TINYINT(1) NULL DEFAULT '0' ");
			$this->db->query("UPDATE `" . DB_PREFIX . "order` set gafenh_ordflag = 1");	
		}
		
		$viewtmp = (substr(VERSION,0,3)=='2.2' || substr(VERSION,0,3)=='2.3') ? '*/template/' : '';
		$this->addtoevent('catalog/controller/common/header/before', $this->modsprtor. 'loadjscss');
	...
	public function uninstall() {
		...
		if(substr(VERSION,0,3)=='3.0' || substr(VERSION,0,3)=='4.0' || substr(VERSION,0,3)=='4.1') {			
	...
	public function loadjscss() {		
		if($this->config->get($this->modname.'_status')) {
			$ocstr = (substr(VERSION,0,3)=='4.0' || substr(VERSION,0,3)=='4.1') ? '../extension/gafenh/admin/' : '';
			$this->document->addScript($ocstr.'view/javascript/gafenh.js?vr='.rand());
			$this->document->addStyle($ocstr.'view/javascript/gafenh.css?vr='.rand());
		}			
	}
...
/*************************************************** INPUT FORMS ***************************************************/	
	...
	public function get_RDO_html($name, $val, $entry, $looparr, $help = '') {	
		$input = array();
		...
		return sprintf('<div class="'.$this->divcls.'"> <label class="col-sm-2 '.$this->lblcls.'">%s</label><div class="col-sm-10"> %s</div> %s </div>', $entry, join($input), $help);
	}
